...
class GraphData(JSONResponseMixin, ListView):
    ...
    def get_queryset(self):
        entries = TimeSerieEntry.objects.select_related('time_serie').filter(
            time_serie__slug=self.kwargs['slug'])
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        for obj in context["object_list"]:
            ...
            if not any(ts['serie'] == serie for ts in time_series):
                title = obj.time_serie.country.name
                time_serie = {"serie": serie, "title": title, "entries": []}
